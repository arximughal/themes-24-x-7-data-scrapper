...
class ThemesSpider(scrapy.Spider):
    ...
    def parse_post(self, response):
        yield {
            ...
            'og:image': response.selector.xpath("//meta[@property='og:image']/@content")[0].extract().replace('themes24x7', 'CestaFina'),
            'twitter:image': response.selector.xpath("//meta[@name='twitter:image']/@content")[0].extract().replace('themes24x7', 'CestaFina'),
            ...
            'og:image': response.selector.xpath("//meta[@property='og:image']/@content")[0].extract().replace('themes24x7', 'CestaFina'),
            'og:image:secure_url': response.selector.xpath("//meta[@property='og:image:secure_url']/@content")[0].extract().replace('themes24x7', 'CestaFina'),
